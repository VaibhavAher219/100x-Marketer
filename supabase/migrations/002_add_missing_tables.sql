-- Add missing tables and columns for job portal system
-- This script safely adds only what's missing

-- Check if employers table exists, if not create it
DO $$ 
BEGIN
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_profiles_user_type') THEN
...
DO $$ 
BEGIN
    -- Check if trigger exists for profiles
    ...
    -- Check if trigger exists for jobs (only if jobs table exists)
